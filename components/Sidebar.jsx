...
const Sidebar = ({ docs }) => {
  ...
  useEffect(() => {
    let mathchedDocs = docs;
    if (pathname.includes("/tags")) {
      const tag = pathname.split("/")[2];
      mathchedDocs = getDocumentsByTag(docs, tag);
    } else if (pathname.includes("/authors")) {
      const author = pathname.split("/")[2];
      mathchedDocs = getDocumentsByAuthor(docs, author);
    } else if (pathname.includes("/categories")) {
      const category = pathname.split("/")[2];
      mathchedDocs = getDocumentsByCategory(docs, category);
    }

    const roots = mathchedDocs.filter((doc) => !doc.parent);
    // console.log({ roots });

    const nonRoots = Object.groupBy(
      mathchedDocs.filter((doc) => doc.parent),
      ({ parent }) => parent
    );
          ...
          <ul role="list" className="border-l border-transparent">
            {rootNodes.map((rootNode) => (
              <li key={rootNode?.id} className="relative">
                ...
                {nonRootNodesGrouped[rootNode?.id.replace(/\.$/, "")] && (
                  <ul role="list" className="border-l border-transparent">
                    {nonRootNodesGrouped[rootNode.id.replace(/\.$/, "")].map(
                      (subRoot) => (
                        <li key={subRoot.id}>
                          <Link
